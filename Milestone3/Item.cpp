...
Item::Item(const std::string& record) {
	...
	if (!name.empty()) {
		if (more) {
			filler = util.nextToken(record, pos, more);
			if (more) {
				remover = util.nextToken(record, pos, more);
				if (more) {
					code = (unsigned int)atoi(util.nextToken(record, pos, more).data());
					if (more) {
						description = util.nextToken(record, pos, more);
					}
				}
			}
		}
		...
		std::string errmsg = record + "<-- *** no token found before the delimiter ***";
		throw errmsg;
	}
}
